...
export const fetchBinData = async () => {
    ...
    const habits = getAtPath(json?.record, path) || [];
    return habits;
};
export const putBinDataForUser = async (userHabits) => {
    const res = await fetch(API_URL, { headers: { "X-Master-Key": API_KEY } });
    ...
    const updatedHabits = [
        ...allHabits.filter((h) => h.userId !== currentUserId),
        ...userHabits.map((h) => ({ ...h, userId: currentUserId })),
    ];
